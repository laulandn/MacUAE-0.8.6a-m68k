		     ...
		     int width, int height, int can_do_blit)
{
    ...
    if (! picasso_vidinfo.extra_mem)
    {
        write_log ("WARNING: do_blit() doesn't have .extra_mem\n");
        return;
    }
    ...
    P96TRACE(("do_blit with srcp 0x%x, dstp 0x%x, src_rowbytes %d, dst_rowbytes %d, srcx %d, srcy %d, dstx %d, dsty %d, w %d, h %d, dst_pixbytes %d\n",
        srcp, dstp, src_rowbytes, picasso_vidinfo.rowbytes, srcx, srcy, dstx, dsty, width, height, picasso_vidinfo.pixbytes));
...
void picasso_refresh (void)
{
	...
	write_log ("picasso_refresh() successful.\n"); /* %%% - BERND, this is misleading.  The do_blit() may have failed! */
    } else
...
uae_u32 picasso_SetPanning (void)
{
    ...
    write_log ("SetPanning(%d, %d, %d) Start 0x%x, BPR %d Bpp %d RGBF %d\n",
...
uae_u32 picasso_InvertRect (void)
{
    ...
    P96TRACE(("InvertRect %d %lx\n", Bpp, (long)mask));

    if (mask != 0xFF && Bpp > 1) {
	write_log ("WARNING: InvertRect() has unhandled mask 0x%x with Bpp %d. Using fall-back routine.\n", mask, Bpp);
    return 0;
    }
 ...
 */
static int BlitRect (struct RenderInfo *ri, struct RenderInfo *dstri,
		      ...
		      uae_u16 width, uae_u16 height, uae_u8 mask)
{
    ...
    if (dstri == NULL) {
        if( mask != 0xFF && Bpp > 1 ) 
        {
            write_log ("WARNING - BlitRect() has unhandled mask 0x%x with Bpp %d. Using fall-back routine.\n", mask, Bpp);
            return 0;
        }
        dstri = ri;
        cant_blit = 0;
...
uae_u32 picasso_BlitRect (void)
{
    ...
    return BlitRect(&ri, NULL, srcx, srcy, dstx, dsty, width, height, Mask);
}
...
uae_u32 picasso_BlitRectNoMaskComplete (void)
{
    ...
    P96TRACE(("BlitRectNoMaskComplete() op 0x%2x, Bpp %d, xy(%4d,%4d) --> xy(%4d,%4d), wh(%4d,%4d)\n",
			  OpCode, Bpp, srcx, srcy, dstx, dsty, width, height));
    P96TRACE(("-- src mem 0x%x BPR %d, dst mem 0x%x BPR %d, screen-mem 0x%x - 0x%x\n",
	...
	return 1;
     ...
     default:
         write_log ("WARNING - BlitRectNoMaskComplete() has unhandled opcode 0x%x\n", OpCode);
	/* FOR NOW! */
	return 0;
    }
}
...
uae_u32 picasso_BlitPattern (void)
{
	...
	if (Bpp > 1)
    {
	    write_log ("WARNING - BlitPattern() has unhandled mask 0x%x with Bpp %d. Using fall-back routine.\n", Mask, Bpp);
        return 0;
    }
	...
	int prow = (rows + pattern.YOffset) & ysize_mask;
...
uae_u32 picasso_BlitTemplate (void)
{
	...
	if (Bpp > 1)
    {
	    write_log ("WARNING - BlitTemplate() has unhandled mask 0x%x with Bpp %d. Using fall-back routine.\n", Mask, Bpp);
        return 0;
    }
...
uae_u32 picasso_BlitPlanar2Direct (void)
{
    ...
    if (Mask != 0xFF) {
	write_log ("WARNING - BlitPlanar2Direct() has unhandled mask 0x%x. Using fall-back routine.\n", Mask );
	return 0;
    }
    if (! CopyRenderInfoStructureA2U (ri, &local_ri)
	|| ! CopyBitMapStructureA2U (bm, &local_bm))

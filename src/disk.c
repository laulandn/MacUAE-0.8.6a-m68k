...
/// Borrowed from gzio.c, !ZAJC! 041098
...
gzFile *DISK_validate_filename (const char *fname, int leave_open, int *wrprot)
{
    //FILE *f = zfile_open (fname, "r+b");
    gzFile *f = gzopen(fname, "r+b");
    if (f) {
	if (wrprot)
	    *wrprot = 0;
    } else {
...
static int drive_insert (drive *drv, int dnum, const char *fname)
{
    ...
    gzread(drv->diskfile, buffer, 8 * sizeof(char)); // sizeof kept, just to be safe!
	...
	// ZLib doesn't support SEEK_END, just assume it's a normal disk for now. :^|
	i = 901120;
...
/*    if (drv->num_tracks <= tr) {
	...
	for (sec = 0; sec < drv->num_secs; sec++) {
	    ...
	    //fseek (drv->diskfile, drv->trackdata[tr].offs + sec*512, SEEK_SET);
	    gzseek(drv->diskfile, drv->trackdata[tr].offs + sec*512, SEEK_SET);
	    
	    //fread (&secbuf[32],1,512,drv->diskfile);
		gzread(drv->diskfile, &secbuf[32], 512);
	...
	gzseek(drv->diskfile, drv->trackdata[tr].offs, SEEK_SET);
	
	//fread((char *)(drv->bigmfmbuf + 1), 1, drv->trackdata[tr].len, drv->diskfile);
	gzread(drv->diskfile, (char *)(drv->bigmfmbuf + 1), drv->trackdata[tr].len);
...
static void drive_write_data (drive *drv, uae_u16 *mbuf, int length)
{
	...
	secwritten++;
	if (!zipped[drv - floppy]) // !ZAJC! 041098
	{
	  fseek ((FILE *) (((gz_stream *) (drv->diskfile)) -> file),drv->trackdata[drv->cyl*2 + side].offs + trackoffs*512, SEEK_SET);
	  fwrite (secbuf+32, sizeof (uae_u8), 512, (FILE *) (((gz_stream *) (drv->diskfile)) -> file) );
	} else
	{
		gzseek (drv->diskfile, drv->trackdata[drv->cyl*2 + side].offs + trackoffs*512, SEEK_SET);
		gzwrite(drv->diskfile, secbuf+32, 512 * sizeof(uae_u8)); // Is this slow?!
    }
	} // end while
    drv->buffered_side = 2; /* will force read */
...
//    fread (dptr, 1, 512, floppy[0].diskfile);
    gzseek(floppy[0].diskfile, floppy[0].trackdata[tr].offs + sec*512, SEEK_SET);
	gzread(floppy[0].diskfile, dptr, 512);
...
void disk_insert (int num, const char *name)
{
    ...
    if (!strstr (name, ".adz") && !strstr (name, ".gz"))
    	zipped[num]=0; // !ZAJC! 041098
    else
    	zipped[num]=1; // !ZAJC! 041098
}

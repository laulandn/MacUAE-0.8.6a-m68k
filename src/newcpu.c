...
static void build_cpufunctbl (void)
{
	...
	if (table68k[opcode].handler != -1) {
	    ...
	    if (f == op_illg_1)
        {
            write_log( "abort(): op_illg_1 in newcpu.c\n" );
		    abort();
        }
...
#endif
    /* %%% - BERND, I've "enhanced" this section for stupid users */
...
unsigned long REGPARAM2 op_illg (uae_u32 opcode)
{
	...
	quit_program = 1;
     	              //printf("op_illg() set quit_program...\n");  // NICKL
   }
    if (opcode == 0xFF0D) {
...
/*	regs_backup[backup_pointer = (backup_pointer + 1) % 16] = regs;*/
    ...
    {
        do_cycles( cycles );
    } while( regs.stopped && !(regs.spcflags & SPCFLAG_INT ) );
...
void m68k_go (int may_quit)
{
	...
	abort ();
          ...
          //printf("past reentrant check in m68k_go()...\n");  // NICKL


    reset_frame_rate_hack ();
    update_68k_cycles ();

          //printf("m68k_go() about to loop...\n");  // NICKL
	...
	if (quit_program > 0) {
	    ...
	    quit_program = 0;
          //printf("about to m68k_reset()...\n");  // NICKL
	    m68k_reset ();
          //printf("about to reset_all_systems()...\n");  // NICKL
	    reset_all_systems ();
          //printf("about to customreset()...\n");  // NICKL
	    customreset ();
          //printf("after customreset()...\n");  // NICKL
	}
	if (debugging)
	    debug ();
          //printf("about to m68k_run1()...\n");  // NICKL
	m68k_run1 ();
    }
    in_m68k_go--;
}
